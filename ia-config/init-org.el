...
(ia/feat-chunk ia-setup/org-mode t
  (with-eval-after-load 'org
    ...
    ;; modes specifies below that have no ob-* installed yet will
    ;; result in error. specify ones that are shipped with org.
    (setopt org-babel-load-languages '((emacs-lisp . t) (shell . t)
                                       (plantuml . t)))

    (ia/feat-chunk ia-setup/org-source-block t
      (setopt org-edit-src-content-indentation 0)
      ;; lowercase instead of uppercase
      (setopt org-babel-results-keyword "results")
      (setopt org-cycle-hide-block-startup t)

      (with-eval-after-load 'ia-org-utility
        (ia/org-remove-blank-line-before-results-mode +1))

      (ia/feat-chunk ia-setup/org-diagram-source t
        ;; Respective diagram executable paths for source blocks such as
        ;; for ob-plantuml are set in each section in `init-diagram.el'.
        ;;
        (setq org-startup-with-inline-images t)
        (setq org-image-max-width 500)
        ;; use `ia/org-redisplay-inline-images-at-block' from
        ;; emacs-utils repo instead of `org-redisplay-inline-images',
        ;; which the former refreshes the image of the current block,
        ;; the latter refreshes across the whole buffer.
        (with-eval-after-load 'ia-org-utility
          (add-hook 'org-babel-after-execute-hook #'ia/org-redisplay-inline-images-at-block)))
      )
